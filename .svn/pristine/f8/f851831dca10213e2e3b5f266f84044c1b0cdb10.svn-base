    ...
    int position, TranslationEntry *pageTable, unsigned numPages)
{
    ...
    oldPageTable = machine->currentPageTable;
    oldNumPages = machine->currentPageTableSize;
    machine->currentPageTable = pageTable;
    machine->currentPageTableSize = numPages;
    for (i = 0; i < numBytes; i++)
        machine->WriteMem(virtualaddr + i, 1, (int)tmp[i]);
    ...
    if (noffH.initData.size > 0)
      {
          ...
          ReadAtVirtual(executable, noffH.initData.virtualAddr,
                noffH.initData.size, noffH.initData.inFileAddr, pageTable,
                numPages);
        //   executable->ReadAt (&
        //                       (machine->mainMemory
...
unsigned AddrSpace::AllocateUserStack() {
    int val;
    mutex->P();
    int threadNumber = bitmap->Find();
    if (threadNumber == -1) {
        mutex->V();
        return 0;
    }
    bitmap->Mark(threadNumber);
    val = numPages * PageSize - threadNumber * 256;
    mutex->V();
    return val;
}

void AddrSpace::DeAllocateUserStack(int stack) {
    int threadNumber = (int)(numPages * PageSize - stack) / 256;
    DEBUG('x', "thread number: %d\n", threadNumber);
    bitmap->Clear(threadNumber);
}

void AddrSpace::augmenterThreadCounter() {
    mutex->P();
    threadCounter++;
    mutex->V();
}

void AddrSpace::diminuerThreadCounter() {
    mutex->P();
    threadCounter--;
    mutex->V();
}
